...
public class PackageBlock extends Block implements BlockEntityProvider {
	private static final Text TITLE = Text.translatable("container.shipit.package");
	...
	public List<ItemStack> getDroppedStacks(BlockState state, LootContextParameterSet.Builder builder) {
		...
		if (be instanceof PackageBlockEntity) {
			...
			if (state.get(PACKAGE_STATE) == PackageState.NEW) {
				...
				if (!pbe.isEmpty()) {
					stack.setSubNbt("Stamps", NbtInt.of(state.get(STAMPS)));
					stack.setSubNbt("PackageState", NbtInt.of(1));
					stack.setSubNbt("BlockEntityTag", pbe.getInventoryTag(new NbtCompound()));
				}
	...
	public BlockState getPlacementState(ItemPlacementContext ctx) {
		NbtCompound tag = ctx.getStack().getNbt();
